...
#' \itemize{
#' \item *X:*	The standardized matrix of x, resulting from using the Prep.Tables() function from the 'staticoex3' library, with the preprocessing argument indicating the type of standardization to be applied.
...
HJSTATICO=function(x,y,condition,bplot,preprocessing,compx,compy) {
  ...
  prep=staticoex3::Prep.Tables(x=x,y=y,condition=condition,preprocessing=tp)
  X=prep$X
  Y=prep$Y
  index=prep$index
  #COINERTIA
